...
export default function DashboardPage() {
  ...
  const [reviewDialog, setReviewDialog] = useState<{ open: boolean; sessionId: number | null }>({ open: false, sessionId: null });
  ...
  useEffect(() => {
    ...
    if (isLoaded && isSignedIn) fetchEnrollments();
  ...
  async function handleSubmitReview() {
    ...
    try {
      const updated = await submitSessionReview(token, reviewDialog.sessionId, {
        rating: reviewRating,
        review: reviewText,
      });
      setEnrollments((prev) => prev.map((e) => (e.id === reviewDialog.sessionId ? updated : e)));
  ...
  if (!isLoaded) {
    return <div className="container py-10"><div className="flex items-center justify-center"><div className="text-lg">Loading...</div></div></div>;
  }

  if (!isSignedIn) { router("/login"); return null; }
    ...
    <div className="container py-10">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {enrollments.map((enrollment) => (
          <div key={enrollment.id} className="rounded-2xl p-6 relative overflow-hidden bg-linear-to-br from-blue-500 to-blue-600 flex flex-col">
            ...
            <div className="size-24 rounded-full bg-white/10 mb-4">
              {enrollment.mentorProfileImageUrl ? (
                <img src={enrollment.mentorProfileImageUrl} alt={enrollment.mentorName}
            ...
            <div className="space-y-1 flex-1">
              ...
              {enrollment.durationMinutes && (
                <p className="text-blue-100/60 text-xs">Duration: {enrollment.durationMinutes} min</p>
              )}
            </div>
                ...
                {enrollment.studentRating ? (
                  <div className="flex items-center gap-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star key={i} className={`h-4 w-4 ${i < enrollment.studentRating! ? "fill-yellow-300 text-yellow-300" : "fill-white/20 text-white/20"}`} />
                    ))}
      ...
      {/* Review Dialog */}
      <Dialog open={reviewDialog.open} onOpenChange={(open) => setReviewDialog({ open, sessionId: open ? reviewDialog.sessionId : null })}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-5 pt-2">
            ...
            {reviewNotification && (
              <p className={`text-sm font-medium ${reviewNotification.includes("success") || reviewNotification.includes("submitted") ? "text-green-600" : "text-red-500"}`}>
                {reviewNotification}
              </p>
            )}
            <div className="flex gap-2 justify-end">
              <Button variant="outline" onClick={() => setReviewDialog({ open: false, sessionId: null })}>Cancel</Button>
